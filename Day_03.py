...
def add_items(item_name, qty):
    if item_name in inventory:
        inventory[item_name] += qty
    else:
        inventory[item_name] = qty
    print(f'{qty} {item_name}(s) added.')

def remove_items(item_name, qty):
    if item_name in inventory:
        if inventory[item_name] >=qty:
            inventory[item_name] -=qty
            print(f'{qty} {item_name}(s) removed.')
        ...
        print(f'{item_name} not found the inventory')
...
def display_inventory():
    if not inventory:
        ...
        for items, q in inventory.items():
            print(f"--{items} : {q}")
...
add_items("Apple", 4)
add_items("Mango", 5)
remove_items("Apple", 3)
remove_items("Mango", 1)
remove_items("Tomoto", 1)
